...
#     adm_team=(
#         db.query(
#             cl.sales_phone_number.label("sales_phone_number"),
#             cl.admin_Team.label("admin_team"),
#             func.count(cl.id).label("team_count")
#         )
#         .group_by(cl.sales_phone_number,cl.admin_Team)
#         .subquery()
#     )

#     adm_json=(
#         db.query(
#             adm_team.c.sales_phone_number,
#             func.json_object_agg(
#                 adm_team.c.admin_team,
#                 adm_team.c.team_count
#             ).label("admin_team_counts")
#         )
#         .group_by(adm_team.c.sales_phone_number)
#         .subquery()
#     )
#     summary_subq=(
#         db.query(
...
#             func.min(cl.call_date).label("from_date"),
#             func.max(cl.call_date).label("to_date"),
#             func.max(cl.uploaded_at).label("last_sync"),
#             func.sum(case((cl.call_type == "missed", 1), else_=0)).label("missed_calls"),
#             func.sum(case((cl.call_type == "incoming", 1), else_=0)).label("received_calls"),
#             func.sum(case((cl.call_type == "outgoing", 1), else_=0)).label("outgoing_calls"),
#             func.max(cl.call_duration).label("total_duration"),
#         )
#         .group_by(cl.sales_phone_number)
#         .subquery()
#     )
#     results = (
#         db.query(
#        summary_subq.c.sales_phone_number,
...
#        func.coalesce(adm_json.c.admin_team_counts, func.cast('{}',JSON)).label("admin_team_counts")
# )
# .outerjoin(adm_json,summary_subq.c.sales_phone_number==adm_json.c.sales_phone_number)
# .all()
#     )
...
#         data.append(
#         {
#             "Sales_Phone_Number": r.sales_phone_number,
...
#             "Received_Calls": r.received_calls,
...
def get_call_log_summary(db: Session = Depends(get_db)):
    ...
    # Step 1: Handling DB connection
    try:
        # Query for admin team summary
        adm_team = (
            db.query(
    ...
    try:
        # Query to aggregate admin team counts
        adm_json = (
            db.query(
    ...
    try:
        # Query to get summary data for call logs
        summary_subq = (
            db.query(
                cl.emp_phone_number.label("emp_phone_number"),
                func.count(cl.id).label("total_calls"),
    ...
    try:
        # Final join and query aggregation
        results = (
            db.query(
            ...
            .outerjoin(adm_json, summary_subq.c.emp_phone_number == adm_json.c.emp_phone_number)
            .all()
        )
    except Exception as e:
        raise HTTPException(
    ...
    # Step 2: Convert results to serializable dicts
    data = []
    try:
        for r in results:
            data.append(
