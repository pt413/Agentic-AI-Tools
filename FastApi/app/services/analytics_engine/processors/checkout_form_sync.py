...
from ..core.config import STAGING_CHECKOUT
from ..processors.sync_support import add_email_participant, add_staff_participant, append_contexts


class CheckoutFormSync(BaseSyncProcessor):
    PROCESSOR_NAME = "checkout_form_sync"
    SOURCE_TABLE = STAGING_CHECKOUT
    SOURCE_TABLE_NAME = "staging_checkout_form"

    def __init__(self, db):
        super().__init__(db, ServiceContainer(db), default_batch_size=3000)

    def fetch_rows(self, last_source_id: int, batch_size: int):
        return self.source.fetch_checkout_forms(last_source_id=last_source_id, batch_size=batch_size)

    def process_row(self, r):
        event_time = self.source.extract_checkout_event_time(r)
        if not event_time:
        ...
        event_meta = {
            ...
            "checkout_date": getattr(r, "checkout_date", None),
            "rms_rating": getattr(r, "rms_rating", None),
            "building_rating": getattr(r, "building_rating", None),
            "refer_friends_score": getattr(r, "refer_friends_score", None),
            "rms_comment": getattr(r, "rms_comment", None),
        ...
        metric_value = getattr(r, "rms_rating", None)
        event_id = self.events.create_event(
            event_family="stay",
            event_name="checkout_completed",
            event_channel="checkout",
            ...
            metric_unit=None,
            metric_name="rms_rating" if metric_value is not None else None,
            meta=event_meta,
        )
        ...
        if event_id:
            participant_data = self.source.extract_checkout_participants(r)
            ...
            if guest_email:
                self.identity.resolve_or_create_person_from_keys(
                    ...
                    merge_reason="checkout_guest_seed",
                )
            ...
            for role_key in ("supervisor", "caretaker", "ops_manager", "salesperson"):
                ...
                self.identity.resolve_or_create_person_from_keys(
                    ...
                    merge_reason=f"checkout_{role_key}_seed",
                )
            ...
            contexts_written += append_contexts(
                self,
                event_id,
                self.source.extract_checkout_contexts(r),
            )
    ...
    def run(self, batch_size=3000, limit=None, start_source_id=None):
        ...
        return {
            "processed_checkout_rows": result["processed_rows"],
