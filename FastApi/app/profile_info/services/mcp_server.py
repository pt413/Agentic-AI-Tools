...
from app.BrightpathAI.utils.external_data import get_call, get_ticket, get_invoice, get_communication, get_booking, get_lead, get_faq_policy_search
from app.services.cache_manager import cache_manager
from google import genai
from app.BrightpathAI.utils.external_data import sales_executive_lead, sales_team_24hr
from app.BrightpathAI.utils.generic_utils import query_executer, semantic_query_executor
...
async def llm_func_call(query: str, system_prompt: str = "support", booking_id: str = None, lead_id: str = None) -> str:
    await EventBroadcaster.publish("🧠 Analyzing query requirements...")
    ...
    async def execute_function_call(fc):
        ...
        try:
            ...
            function_messages = {
                "get_invoice": "📄 Fetching invoice...",
                "get_booking": "📋 Loading booking...",
                "get_communication": "💬 Getting messages...",
                "get_call": "📞 Accessing calls...",
                "get_ticket": "🎫 Loading tickets...",
                "get_lead": "👥 Getting lead data...",
                "get_faq_policy_search": "📚 Searching FAQs...",
                "sales_executive_lead": "📊 Loading Sales data...",
                "query_executer": "🛢️ Database query...",
                "sales_team_24hr": "👨‍💼 Team data..."
            }
            
            progress_msg = function_messages.get(fc.name, f"🔄 Getting {fc.name}...")
            await EventBroadcaster.publish(progress_msg)
            ...
            if fc.name == "get_invoice":
                # await EventBroadcaster.publish("loading invoice")
    ...
    try:
        await EventBroadcaster.publish("Preparing result...")
...
def smart_llm_call(contents, config):
    ...
    resp = safe_generate(PRIMARY, contents, config)  
    if resp:
        return resp

    print("⚠ Primary unavailable → switching to BACKUP")
    resp = safe_generate(BACKUP, contents, config)  
    if resp:
        return resp

    print("⚠ Backup also failed → switching to SAFE model")
    resp = safe_generate(SAFE, contents, config)  
    if resp:
        return resp
